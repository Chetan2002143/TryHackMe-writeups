...
The database checks if you exist and whether that password matches. If yes, you’re in! But hackers have a trick up their sleeve called SQL Injection that lets them twist this query into something that works for them, not the website.
...
Username: John
Password: abc’ OR 1=1;– -
That last part after “abc” is the secret sauce. The database now sees this query:

SELECT * FROM users WHERE username = 'John' AND password = 'abc' OR 1=1; - -';
What’s going on here? The password part of the query now reads:

Password = ‘abc’ (nope, that’s incorrect), but wait, then it checks:
OR 1=1 (which is always true!)

So, the database just shrugs and lets them in. And voilà — the attacker gets access to John’s account without needing a clue about his actual password. The double dashes — tell the database to ignore anything after, essentially leaving out the real password check.
...
Feeling like hacking databases is too much typing? Enter SQLMap, the database burglar’s best friend. Forget trying to break in manually — SQLMap does the hard work for you. It’s like a tour guide for database heists, handling everything from picking the lock to finding the treasure.

SQLMap is the command-line tool that turns you into a database wizard, as long as you have permission (seriously). Start by launching it in your Linux terminal, and if you’re feeling overwhelmed, try — wizard mode. Think of it as SQLMap holding your hand and walking you through the basics.

Want to test a URL for weakness? Just feed it a target URL, like http://sqlmaptesting.thm/search?cat=1, and let SQLMap dig in. If it finds a weak point, it’ll reveal the details: error-based, time-based, UNION query, you name it — SQLMap’s got receipts.

Once you’ve confirmed a vulnerable URL, SQLMap’s Swiss Army knife of flags is ready. Run — dbs to see the databases, -D to pick one, and — tables to browse the goods inside. Found something juicy? Use — dump to get all the records. In a few commands, you’re a digital Indiana Jones uncovering the secrets of SQL!

Too good to be true? Well, yes — unless you’ve got the admin’s blessing. Happy hacking responsibly!
...
Start the virtual machine and AttackBox, then visit the login page at http://MACHINE_IP/ai/login. This page is vulnerable to SQL injection.
...
You can explore further with specific SQLMap flags to retrieve data from the database if required.

By following these steps, you’ll successfully test SQL injection vulnerabilities on the login page.
